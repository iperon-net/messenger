...
/// видишь чужое присутствие (кроме премиум) — поясняем внизу.
class SettingsPrivacyLastSeenMaterial extends StatelessWidget {
  const SettingsPrivacyLastSeenMaterial({super.key});
  ...
  /// Открывает пикер списка-исключений ([kind]) для канала «последнее посещение»;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<SettingsPrivacyAndSecurityCubit, SettingsPrivacyAndSecurityState>(
      builder: (context, state) {
        final muted = Theme.of(context).colorScheme.onSurfaceVariant;
        return Scaffold(
          appBar: AppBar(title: Text(context.t.sessionsPrivacyAndSecurity.lastSeen)),
                      ...
                      if (state.callsLoadError)
                        ...
                        for (final audience in CallsPrivacyAudience.values)
                          ListTile(
                            ...
                            trailing: state.lastSeenAudience == audience ? check : null,
                          ),
                    ],
                  ),
                ),
