                    ...
                    children: [
                                ...
                                child: CupertinoTextFormFieldRow(
                                  ...
                                  validator: (value) {
                                    final error = switch (context.read<AuthCubit>().validatePhoneNumber(value)) {
                                      ...
                                      PhoneValidationError.notAllowRegion => context.t.auth.currentlyWeOnlySupportPhoneNumbersFromRussianMobileOperators,
                                      null => null,
                                    };
                              ...
                              if (kDebugMode) ...[
                                ...
                                FilledButton.tonal(
                                  child: Text(t.auth.insertDebugPhone),
                                    onPressed: () {
                                      phoneNumberController.text = utils.phoneNormalization(
                                        phoneNumber: settings.phoneNumberModerationApplicationStore,
                                      ).international;
                                    }
                                ),
                              ]
                      ...
                      SizedBox(
                        ...
                        child: CupertinoButton.filled(
                          ...
                          onPressed: [Status.success, Status.initialization].contains(state.status) ? () async {
                            serverError = null;
                            if (formKey.currentState!.validate()) {
                              await context.read<AuthCubit>().onPressed(phoneNumberController.text);
                            }
                          } : null,
                          child: [Status.success, Status.initialization].contains(state.status) ? Text(context.t.common.kContinue) : CupertinoActivityIndicator(color: Color(0xffffffff)),
                        ),
                      ),
