...
class Users {
  ...
  Future<void> createOrUpdate({
    ...
    await db.writeTransaction((txn) async {
      final sqlUser = await txn.execute(
        "SELECT userID FROM users WHERE phoneNumber = ?;",
        [phoneNumber],
      );
      if (sqlUser.isEmpty) {
        await txn.execute(
          "INSERT INTO users (userID, phoneNumber) VALUES(?, ?);",
          [userID, phoneNumber],
        );
      } else {
        await txn.execute("UPDATE users SET phoneNumber = ? WHERE userID = ?", [
          phoneNumber,
          userID,
        ]);
      }
  ...
  Future<User> getBySession({required Session session}) async {
    ...
    final sqlUser = await db.execute(
      "SELECT userID, phoneNumber FROM users WHERE userID = ? LIMIT 1;",
      [session.userID],
    );
  ...
  Future<void> setSalt({
    ...
    await db.execute("UPDATE users SET salt = ? WHERE userID = ?;", [
      salt,
      session.userID,
    ]);
  }
