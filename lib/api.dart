...
class API {
  ...
  Future<void> shutdown() async {
    ...
    // [incoming] — при обычном разлогине он остаётся жив (см. [_close]).
  ...
  /// Закрытие соединения (logout/деавторизация): сносит gRPC-стрим и отменяет
  /// всё запланированное, но **оставляет broadcast [incoming] живым**.
  ///
  /// Раньше `_close()` закрывал и обнулял `_incoming` — и это ломало входящие
  /// звонки: `Calls` подписывается на `api.on(CALL_RING)` один раз в
  /// конструкторе, а на старте `_reconcile` успевает вызвать `_close()` (сессия
  /// ещё грузится, `!_authorized`) — старый контроллер уничтожался, а следующий
  /// `_open()` создавал новый, и подписка `Calls` навсегда оставалась на мёртвом
  /// контроллере. Контроллер должен жить столько же, сколько синглтон [API].
  /// Доставка сообщений чужой сессии при этом безопасна: [_dispatch] гейтится на
  /// `auth.isAuthorized` и расшифровывает активной сессией.
